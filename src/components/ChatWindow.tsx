...
const ChatWindow = ({
  ...
  return (
    <div
      className={
        "border-translucent flex w-full flex-col rounded-2xl border-2 border-white/20 bg-primary text-white shadow-2xl drop-shadow-lg " +
        (className ?? "")
      }
    >
      ...
      {displaySettings && (
        <div className="flex flex-col items-center justify-center md:flex-row">
          ...
          <SwitchContainer label="Pause Mode">
            <Switch
              disabled={agent !== null}
...
const SwitchContainer = ({
  ...
  return (
    <div className="m-1 flex w-40 items-center justify-center gap-2 rounded-lg border-[2px] border-white/20 bg-secondary px-2 py-1">
...
const MacWindowHeader = (props: HeaderProps) => {
  ...
  return (
    <div className="flex items-center gap-1 overflow-visible rounded-t-3xl p-3">
      ...
      <Expand
        delay={1}
        className="invisible flex flex-grow font-mono text-sm font-bold text-white sm:ml-2 md:visible"
      >
        {props.title}
      </Expand>
      ...
      {agentMode === PAUSE_MODE && agent !== null && (
        ...
        >
          {isAgentPaused ? (
            <>
              <FaPause />
              <p className="font-mono">{t("paused")}</p>
            </>
          ) : (
            <>
              <FaPlay />
              <p className="font-mono">{t("running")}</p>
            </>
          )}
        </div>
      )}
      ...
      <Menu
        ...
        styleClass={{
          container: "relative",
          input: `bg-secondary animation-duration text-left py-1 px-2 text-sm font-mono rounded-lg text-gray/50 border-[2px] border-white/30 font-bold transition-all sm:py-0.5 hover:border-[#1E88E5]/40 hover:bg-[#6b6b6b] focus-visible:outline-none focus:border-[#1E88E5]`,
